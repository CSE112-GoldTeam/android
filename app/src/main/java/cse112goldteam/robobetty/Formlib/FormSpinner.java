package cse112goldteam.robobetty.Formlib;
...
public class FormSpinner extends FormWidget
{
	...
	public FormSpinner( Context context, String property, JSONArray options )
	{
		...
		try{
			...
			{
				name =  _options.getString(i);
				//p = options.getString( name );
				System.out.println(name);
				_adapter.add( name );
				_propmap.put( name, name );
			}
	...
	public void setValue(String value)
	{
		try{
			...
			for( int i = 0; i < names.length(); i++ )
			{
				...
				{
					String item = _options.getString(i);
